...
HT.Hexagon.prototype.getFillPoints = function() {
	var self = this;
	if (!this.fillPath) {
		return null;
	} else {
...
HT.Hexagon.prototype.draw = function(ctx) {
	ctx.save();
	ctx.strokeStyle = this.selected ? "black" : "grey";
	ctx.lineWidth = 1;
	ctx.beginPath();
...
HT.Hexagon.setRegularSize = function (length) {
  this.Static.SIDE = length;
  this.Static.WIDTH = length * 2;
  this.Static.HEIGHT = Math.sqrt(3) * length;
}
